...
/**
 * Draws one organism centered at (x, y), facing right, `size` across. `alpha` fades newborns in
 * and the dead out.
 */
export function drawGlyph(
  ...
  for (let i = 0; i < pairs; i++) {
    ...
    g.moveTo(lx, y - bodyH * 0.6).lineTo(lx - size * 0.05, y - bodyH - reach * 0.6);
    g.moveTo(lx, y + bodyH * 0.6).lineTo(lx - size * 0.05, y + bodyH + reach * 0.6);
  }
  ...
  if (traits[H] > 0) {
    ...
    g.moveTo(fx, y - bodyH * 0.25).lineTo(fx + len, y - bodyH * 0.05 - len * 0.35);
    g.moveTo(fx, y + bodyH * 0.25).lineTo(fx + len, y + bodyH * 0.05 + len * 0.35);
