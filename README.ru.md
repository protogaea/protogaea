...
| B | Наблюдение: карта, диаграмма Мюллера, древо клад, машина времени, сводки | Идёт: готовы сервер мира, вьювер с картой, имена клад, диаграмма Мюллера, древо клад, истории дня, сводка «Пока тебя не было», сутки за 30 секунд, прогнозы на завтра, телеграм-бот и машина времени (любая прошлая эпоха пересчитывается в браузере и сверяется с журналом, сравнение двух моментов, проверка доказательств включения в браузере), музей и ежедневная хроника; дальше подписка на регион и тесты с людьми |
| B′ | Закрытый тест наблюдения с приглашёнными, без PoW | Запланирован |
| C | Искры: yespower, настольное приложение, WASM-клиент, журнал искр, наблюдатели | Начат: yespower с параметрами искры (в том числе на Rust для браузера) протокол искр в коде, приём искр на сервере с журналом и квитанциями клиенты искр в командной строке и в браузере и чудеса: учёт выбирает до трёх за эпоху, ядро их применяет; подписанные заголовки эпох и независимый наблюдатель; дальше маяк и зерно эпохи от хеша заголовка |
...
- [pow/](pow/README.md) — доказательство работы для искр: yespower 1.0 с параметрами искры, вход искры и проверка цели, с замерами.
- [protocol/](protocol/README.md) — протокол искр v0: желания и их подписи, искры и их проверка, журнал искр с доказательствами, подписанные головы дерева и квитанции.
- [spark/](spark/README.md) — клиент искр в командной строке: ключ натуралиста, желания и майнинг, каждая квитанция проверяется.
- [watcher/](watcher/README.md) — независимый наблюдатель: следит за журналом искр и подписанными заголовками, переигрывает мир и поднимает тревогу при любом расхождении.
...
- [viewer/](viewer/README.md) — веб-вьювер (TypeScript и PixiJS): иллюстрированная карта с живыми организмами, шкала сезона, карточки клад и особей, диаграмма Мюллера, древо клад, лента событий и машина времени с ядром, собранным в WebAssembly (`viewer/wasm`), на русском и английском.

Нужны стабильный Rust и Node.js для вьювера. Сервер собирает SQLite из исходников, а `pow` — yespower, поэтому им нужен компилятор C (на Windows — MSVC или gcc из MSYS2); обычный `cargo build` его пропускает.
