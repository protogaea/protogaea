...
| C | Sparks: yespower, desktop app, WASM client, spark log, watchers | Started: yespower with the spark parameters (in Rust too, for the browser) the spark protocol in code, spark intake in the server with its log and receipts, spark clients on the command line and in the browser, and miracles: the ledger selects up to three an epoch and the core applies them; signed epoch headers and an independent watcher; next are the beacon and the epoch seed from the header hash |
...
- [spark/](spark/README.md): the spark client on the command line: a naturalist's key, wishes and mining, every receipt checked.
- [watcher/](watcher/README.md): an independent watcher: follows the spark log and the signed headers and replays the world, raising an alarm at any discrepancy.
...
You need stable Rust, and Node.js and the `wasm32-unknown-unknown` Rust target for the viewer. The server bundles SQLite and `pow` builds yespower, so both need a C compiler (on Windows, MSVC or MSYS2's gcc); a plain `cargo build` leaves it out.
