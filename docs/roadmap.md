...
| # | Decision | Status |
|---|---|---|
| 1 | Final PoW algorithm and parameters, after benchmarks on x86-64, ARM64 and GPUs | yespower 1.0 (N = 2048, r = 32) measured on two x86-64 CPUs with its reference vectors, and ported to Rust for WebAssembly (about 0.45 of the C speed in V8) ([pow](../pow/README.md)); ARM64 open. GPUs are not measured: yespower is built for CPUs |
...
**B7, part 1 — the time machine (2026-09-26):** any past epoch opens in the viewer. Epochs between the server's snapshots are recomputed in the browser by the core compiled to WebAssembly ([`viewer/wasm`](../viewer/wasm/src/lib.rs)), resumed from the nearest earlier snapshot in a worker, and the result's `state_root` is checked against the epoch header in the server's log; 35 epochs take about a second. The season timeline opens any moment and the arrow keys step through epochs. Part 2 (the same day): comparing two moments (what changed between them: archetypes, clades, the leader, the named clades that appeared and disappeared, the stories in between), checking an organism's inclusion proof in the browser with the same core, and the stories and the event feed of a past epoch. The same day the museum of extinct named clades and the daily chronicle, written from templates, closed B7.

**B5, part 1 — coming back (2026-09-26):** the viewer greets a returning viewer with "While you were away" (the population and the leading clade then and now, clades named and extinct, land bridges closed and the best stories, from `/v0/digest`), replays the last world day in about 30 seconds with the day's stories as captions (the server keeps compact frames of the last two world days at `/v0/replay`), and takes predictions for tomorrow on a clade's card ("will it still be alive this time tomorrow?", "will it be larger?"), checked the next day and scored in the browser. The Telegram bot ([`server/bot/`](../server/bot/README.md)) sends the morning digest and tells followers of a clade when it gets its name, leads the world, goes extinct or is in a story. Following a continent in the bot closed B5 the same day.
...
**C, part 1 — proof of work and the spark protocol (2026-09-26):** yespower 1.0 with the spark parameters, checked against its reference vectors and measured on two x86-64 CPUs, and ported to plain Rust for the browser ([`pow`](../pow/README.md)); the spark protocol v0 in code: wishes and their signatures, sparks, the spark log after RFC 9162 with inclusion and consistency proofs, signed tree heads and receipts, with test vectors ([`protocol`](../protocol/README.md)); spark intake in the world server, with the open window, rate limits, bans, the log in its own database and work added to wishes at each close ([server](../server/README.md#wishes-and-sparks-stage-c)); and a command-line spark client that makes weather wishes and mines sparks, checking every receipt ([`spark`](../spark/README.md)). End to end on a local world: 449 sparks in a minute on four threads, each receipt checked, a forged spark refused and its sender banned. Part 2 (2026-09-27): miracles in the core (`weather`, `migrate`, `revive`, with the hard checks and cooldowns of spec §5; cooldowns enter `state_root` only while one runs, so earlier roots hold), and the ledger of spec §19 in the server: price, queue, selection of up to three miracles an epoch, the soft check, the miracle log. End to end on a local world, a rain wish gathered its work in a minute, was selected when its window closed and fell on the next epoch; the time machine recomputed the epochs after it in the browser and matched the log. Part 3 (2026-09-27): the spark client in the browser: a naturalist key, weather wishes by a click on the map, sparks on web workers with yespower in WebAssembly, receipts checked; from the viewer, a rain wish gathered its price in half a minute on eight workers and happened. The same day `migrate` and `revive` came to the viewer (a clade's card, a museum exhibit) and the spark client; on a local world a migration made in the viewer and two revivals happened. Then signed epoch headers with the ledger root, chained by hash; the time machine checks them in the browser. Then the watcher ([`watcher`](../watcher/README.md)): it follows the tree heads and the signed headers and replays the world, and caught a changed header, a removed miracle and dropped sparks on a local world. Next: the epoch seed committing to the header hash, the beacon (drand), and the ledger root checked by watchers.
